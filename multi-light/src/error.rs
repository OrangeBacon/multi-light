...
impl Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::YAMLScanError { err, file_name } => writeln!(
                f,
                "Error while parsing YAML file `{}`: {err}",
                file_name.display()
            ),
            Error::YAMLDocumentCount { count, file_name } => writeln!(
                f,
            ...
            Error::YAMLComplexKey { file_name } => writeln!(
                f,
                "Non-string map key found within YAML file `{}`",
                file_name.display()
            ),
            Error::YAMLAlias { file_name } => writeln!(
                f,
